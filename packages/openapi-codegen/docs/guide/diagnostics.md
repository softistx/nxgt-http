...
| `name_collision` | error | two schemas, or a schema and an operation's parameters or form, would generate the same name; a schema named `Operations`, `ClientOperations`, `OperationsByRoute`, `PathsByMethod`, `OperationsByTag` or `PathsByTag`; with `hono`, one named `Replies`, `HonoSpec` or `Hono`; with `dates: 'date'`, one named `Wire` | rename one with [`names`](options.md#names) |
