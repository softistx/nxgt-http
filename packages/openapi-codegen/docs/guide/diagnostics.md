...
| `not_enforced` | warning | a keyword the validator does not check: `uniqueItems`, `minProperties`, an `additionalProperties: false` that an `allOf` loosens, an `unevaluatedProperties: false` over a `$ref` that accepts unknown keys…; with `hono`, a status Hono has no type for, whose replies are typed with any status | enforce it in code if it matters; for `allOf`, set `unknownKeys: 'strict'` or make every member strict, so unknown keys are refused (keys another member declares are still accepted) |
