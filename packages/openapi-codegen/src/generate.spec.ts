...
describe('generate', () => {
	it('writes the files, then leaves them alone when nothing changed', async () => {
		...
		expect(await readFile(join(cwd, 'gen/types.ts'), 'utf8')).toStartWith(
	...
	it('reports drift in check mode without writing', async () => {
		...
		await writeFile(join(cwd, 'gen/zod.ts'), '// edited\n');
	...
	it('writes to generated/openapi, relative to cwd, when no output is given', async () => {
		const cwd = await project();
		const { written } = await generate({ input: 'openapi.json' }, { cwd });
